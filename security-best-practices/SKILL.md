...
**Note:** For general best practices (type safety, code quality, documentation), use the respective skills. This section focuses exclusively on security-specific anti-patterns.
...
1. **Audit Mode** - Skill invoked directly (`/security-best-practices <path>`) or user explicitly requests security audit
...
**When generating audit reports** (when skill is invoked directly via `/security-best-practices <path>` or user explicitly requests security audit), use the structured template:
