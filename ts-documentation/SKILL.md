...
- Performance optimization (use ts-performance instead)
...
When users explicitly request a documentation audit or invoke the skill directly (`/ts-documentation <path>`), use the standardized report format:
...
- Skill invoked directly via `/ts-documentation <path>`
