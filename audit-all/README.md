...
`audit-all` is a meta-skill that orchestrates four specialized TypeScript audit skills (`ts-testing`, `ts-best-practices`, `ts-performance`, `ts-documentation`) in a systematic 8-step process. It maintains detailed progress tracking across sessions and requires interactive approval for all changes.
...
**npm**
```bash
npx skills add https://github.com/gohypergiant/agent-skills --skill audit-all
```

**pnpm**
```bash
pnpm dlx skills add https://github.com/gohypergiant/agent-skills --skill audit-all
```
...
```bash
/skill audit-all path/to/file.ts
```

Or for an entire directory:

```bash
/skill audit-all path/to/src
```
...
1. Run `ts-testing` to check test quality
2. Interactively apply test improvements

### Phase 2: Code Quality & Performance
3. Run `ts-best-practices` AND `ts-performance` in parallel
...
7. Run `ts-documentation` to improve code comments
...
**Initial invocation:**
```bash
User: /skill audit-all packages/core/src
...
- `ts-testing` - Test coverage and quality analysis
- `ts-best-practices` - Code quality and type safety checks
- `ts-performance` - Performance optimization analysis
- `ts-documentation` - JSDoc and comment quality
...
- [ts-testing](../ts-testing/) - Test quality skill
- [ts-best-practices](../ts-best-practices/) - Code quality skill
- [ts-performance](../ts-performance/) - Performance skill
- [ts-documentation](../ts-documentation/) - Documentation skill
