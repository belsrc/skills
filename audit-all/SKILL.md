---
name: audit-all
description: Comprehensive TypeScript file audit system. Command-only skill (no natural triggers). Accepts file or directory path to systematically audit through ts-testing, ts-best-practices, ts-performance, and ts-documentation skills. Maintains progress tracking across sessions with interactive change approval.
metadata:
  version: "1.0"
---
...
- **NEVER skip the initial test coverage step** - Refactoring without test coverage first leads to undetected breakage. Always run `ts-testing` before any code changes.
...
- **How many files will this audit?** Large directories (>20 files) will require multiple sessions. Set expectations upfront.
...
**Step 1: Run ts-testing skill**
```bash
/skill ts-testing <file-path>
```
...
```bash
/skill ts-best-practices <file-path>
/skill ts-performance <file-path>
```
...
**Step 7: Run ts-documentation skill**
```bash
/skill ts-documentation <file-path>
```
