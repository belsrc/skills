...
1. **Audit Mode** - Skill invoked directly (`/ts-performance <path>`) or user explicitly requests performance audit
...
**When generating audit reports** (when skill is invoked directly via `/ts-performance <path>` or user explicitly requests performance audit), use the structured template:
